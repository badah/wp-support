...
if ( class_exists( 'acf_field' ) ) {
	...
	class Button extends \acf_field {
		...
		function render_field( $field ) {
			?>
			<style>
				.acf-field-option-button .acf-label{
					display:none
				}
				</style>
			<input
				type="button" value="<?php echo $field['label']; // WPCS: XSS ok. ?>"
				class="button button-primary button-large"
				id="<?php echo $field['_name']; // WPCS: XSS ok. ?>"
				name="<?php echo $field['_name']; // WPCS: XSS ok. ?>"
			>
			<?php
		}
	}
